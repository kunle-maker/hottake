...
export const CommentsModal: React.FC<CommentsModalProps> = ({ post, onClose, onAddComment }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white dark:bg-[#0C1D38] text-slate-900 dark:text-white rounded-t-2xl sm:rounded-2xl w-full max-w-lg h-[80vh] flex flex-col shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        {/* Modal Header */}
        ...
        <div className="p-3 bg-slate-50 dark:bg-[#07152B] border-b border-slate-200 dark:border-slate-800 text-xs">
          <span className="font-bold text-[#00A3E0]">@{post.author.username}:</span>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {commentsList.map((c) => (
            <div key={c.id} className="p-3 bg-slate-50 dark:bg-[#07152B] rounded-xl border border-slate-200/80 dark:border-slate-800 space-y-1.5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <img src={c.userAvatar} alt={c.userDisplayName} className="w-7 h-7 rounded-full object-cover" />
                  <div>
                    <div className="flex items-center gap-1">
                      <span className="font-bold text-xs">{c.userDisplayName}</span>
        ...
        {/* Comment Input */}
        <form onSubmit={handleSubmit} className="p-3 bg-slate-100 dark:bg-[#07152B] border-t border-slate-200 dark:border-slate-800 flex gap-2">
          <input
            type="text"
            value={commentText}
            onChange={(e) => setCommentText(e.target.value)}
            placeholder="Write your counter take..."
            className="flex-1 bg-white dark:bg-[#0C1D38] border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2 text-xs"
          />
          <button
            type="submit"
